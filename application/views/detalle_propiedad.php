...
<body>
	...
	<div id="wrapper">	
		...
		<section>			
			...
			#var_dump($imagenes_db);
			$counter_imagenes =0;
			foreach ($imagenes_db as $value) {
				if($counter_imagenes==0){
					$imagen_principal =  '<img src="'.base_url('images-propiedades/'.$value->filename).'" alt="">';
				}
				$counter_imagenes++;
			...
			*/
			...
			if(!empty($imagenes_db)){
				...
				foreach ($imagenes_db as $imagen) {
					if($counter_imagenes!=0){
						...
						<img src="'.base_url('images-propiedades/'.$imagen->filename).'" alt="">
						</div><!-- cada_foto_extra -->
						';
					}
					$counter_imagenes++;
			...
			?>
				...
				<div id="img_bt_vermasfotos">
					...
					if($propiedad->vendido==1){$vendido = '<div class="cartel_rv vendido"></div>';}
					if($propiedad->reservado==1){$reservado = '<div class="cartel_rv reservado"></div>';}

					echo $vendido.''.$reservado.''.$imagen_principal; ?>
				...
				<div id="detalles_derecha_foto">
					...
					echo '<h1>'.$direccion.'<span> '.$tipo_propiedad.' - '.$tipo_transaccion.'</span></h1>'; 
					
					$propiedad_barrio = $this->barrio->get_record( $propiedad->barrio );
					/* BARRIO */
					$barrio = str_replace("CABA - ", "", $propiedad_barrio->nombre);
			...
			<div id="descripcion_detallada">
				...
				<div class="grupo_info_detalle">
					...
					<ul>
						...
						*/
						...
						';
						if($propiedad->sup_lote){echo '<li>Sup. Lote: <span>'.$propiedad->sup_lote.' m2</span> </li>';}
						if($propiedad->sup_cubierta){echo '<li>Sup. cubierta: <span>'.$propiedad->sup_cubierta.' m2</span> </li>';}
						if($propiedad->sup_descubierta){echo '<li>Sup. descubierta: <span>'.$propiedad->sup_descubierta.' m2</span> </li>';}
						if($propiedad->sup_semi_cubierta){echo '<li>Sup. semi-cubierta: <span>'.$propiedad->sup_semi_cubierta.' m2</span> </li>';}
					...
					$hay_info=false; 
					$lista_descripcion = "";
					if(!empty($propiedad->palier_privado)){ $hay_info=true; $lista_descripcion .= '<li>Palier:<span>'.$propiedad->palier_privado.'</span> </li>';}
					...
					if(!empty($propiedad->cant_banos)){ $hay_info=true; $lista_descripcion .= '<li>Cantidad ba&ntilde;os:<span>'.$propiedad->cant_banos.'</span> </li>';}
					...
					if(!empty($propiedad->comedor)){ $hay_info=true; $lista_descripcion .= '<li>Comedor:<span>'.$propiedad->comedor.'</span> </li>';}
					if(!empty($propiedad->lavadero)){ $hay_info=true; $lista_descripcion .= '<li>Lavadero:<span>'.$propiedad->lavadero.'</span> </li>';}
					if(!empty($propiedad->hab_servicio)){ $hay_info=true; $lista_descripcion .= '<li>Hab_servicio:<span>'.$propiedad->hab_servicio.'</span> </li>';}
					if(!empty($propiedad->balcon)){ $hay_info=true; $lista_descripcion .= '<li>Balcon:<span>'.$propiedad->balcon.'</span> </li>';}
					if(!empty($propiedad->segundo_balcon)){ $hay_info=true; $lista_descripcion .= '<li>Patio:<span>'.$propiedad->segundo_balcon.'</span> </li>';}
					...
					if(!empty($propiedad->family)){ $hay_info=true; $lista_descripcion .= '<li>Family:<span>'.$propiedad->family.'</span> </li>';}
					if(!empty($propiedad->playroom)){ $hay_info=true; $lista_descripcion .= '<li>Playroom:<span>'.$propiedad->playroom.'</span> </li>';}
					if(!empty($propiedad->escritorio)){ $hay_info=true; $lista_descripcion .= '<li>Escritorio:<span>'.$propiedad->escritorio.'</span> </li>';}
				...
				<div class="grupo_info_detalle">
					...
					<ul>
						...
						if(!empty($propiedad->calefaccion)){ echo '<li>Calefaccion:<span>'.$propiedad->calefaccion.'</span> </li>';}
						...
						if(!empty($propiedad->doble_circulacion)){ echo '<li>Doble circulacion:<span>'.$propiedad->doble_circulacion.'</span> </li>';}
						if(!empty($propiedad->orientacion)){ echo '<li>Orientacion:<span>'.$propiedad->orientacion.'</span> </li>';}
						if(!empty($propiedad->condicion)){ echo '<li>Otros:<span>'.$propiedad->condicion.'</span> </li>';}
						...
						if(!empty($propiedad->telefono)){ echo '<li>Telefono:<span>'.$propiedad->telefono.'</span> </li>';}
						if(!empty($propiedad->antiguedad)){ echo '<li>Antiguedad:<span>'.$propiedad->antiguedad.'</span> </li>';}
