<?php  
				...
				foreach ($barrios as $barrio) {
					...
					$barrios_div .= '<div class="boxbarrio"><input type="checkbox" name="barrio[]" value="'.$barrio->id.'" '.$checked_barrio .'>'.$barrio->nombre.'</div>';
					
					$barrios_filtro .= '<div class="boxbarrio"><input type="checkbox" name="barrio[]" value="'.$barrio->id.'" '.$checked_barrio .'>'.$barrio->nombre.'</div>';
					
					/*
					if (strpos($barrio,'CABA') !== false) {
						$barrio_clean = str_replace('CABA - ', "", $barrio->nombre);
						if(in_array($barrio, $this->session->userdata('filtro_barrio'))){$checked_barrio = "checked"; }
						$barrios_caba .= '<div class="boxbarrio"><input type="checkbox" name="barrio[]" value="'.$barrio->id.'" '.$checked_barrio .'>'.$barrio_clean.'</div>';
					}
					if (strpos($barrio,'GBA') !== false) {
						$barrio_clean = str_replace('GBA - ', "", $barrio->nombre);
						if(in_array($barrio, $this->session->userdata('filtro_barrio'))){$checked_barrio = "checked"; }
						$barrios_gba .= '<div class="boxbarrio"><input type="checkbox" name="barrio[]" value="'.$barrio->id.'" '.$checked_barrio .'>'.$barrio_clean.'</div>';
				...
				$array_barrios_caba = $this->barrio->get_por_localidad('CABA');

				foreach ($array_barrios_caba as $barrio) {
					$barrio_clean = str_replace('CABA - ', "", $barrio->nombre);
						if(in_array($barrio, $this->session->userdata('filtro_barrio'))){$checked_barrio = "checked"; }
						$barrios_caba .= '<div class="boxbarrio"><input type="checkbox" name="barrio[]" value="'.$barrio->id.'" '.$checked_barrio .'>'.$barrio_clean.'</div>';
				}

				$array_barrios_gba = $this->barrio->get_por_localidad('GBA');
				$barrios_gba="";
				foreach ($array_barrios_gba as $barrio) {
					$barrio_clean = str_replace('GBA - ', "", $barrio->nombre);
						if(in_array($barrio, $this->session->userdata('filtro_barrio'))){$checked_barrio = "checked"; }
						$barrios_gba .= '<div class="boxbarrio"><input type="checkbox" name="barrio[]" value="'.$barrio->id.'" '.$checked_barrio .'>'.$barrio_clean.'</div>';
				}
	...
	<div id="buscador_general">
		...
		<div id="content_form_buscador_general">
			...
			<form id="form_buscador_general" name="form_buscador_general" method="post" action="<?php echo base_url('busqueda'); ?>">
				...
				<div class="grupo_info_form">
			    	<label for="barrio" id="barrio_localidad_busc_gral" class="nombre_campos_buscador_general">BARRIO</label><br />
			    ...
			    <div class="grupo_info_form texto_moneda">
			    	...
			    	<input type="radio" id="moneda" name="moneda" value="$"<?php if( trim($this->session->userdata('filtro_moneda')) == '$'){echo "checked";} ?>>$
					<input type="radio" id="moneda" name="moneda" value="u$s" <?php if(trim($this->session->userdata('filtro_moneda')) == 'u$s'){echo "checked";} ?>>u$s
				</div><!-- grupo_info_form -->			    
				<!--  DIV CON BARRIOS	 -->		    
